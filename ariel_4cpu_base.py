...
ch=int(g_params["numChannels"])
rank=int(g_params["numRanksPerChannel"])
bg=int(g_params["numBankGroupsPerRank"])
bank=int(g_params["numBanksPerBankGroup"])
row=int(g_params["numRowsPerBank"])
col=int(g_params["numColsPerBank"])
cl=int(g_params["numBytesPerTransaction"])


memory_size = ch*rank*bg*bank*row*col*cl/1024/1024/1024   #GB
print "memory_size:%d"%memory_size
pagesize=4096       #B
pagecount = memory_size*1024*1024*1024/corecount/pagesize
print "page count:%d"%pagecount
compression_en = 0
...
memory.addParams({
  ...
  "backend.max_outstanding_requests" : 2048,
    "backend.verbose" : 0,
...
# Bus to CPU to MEM for trasferring memory contents
memcontentbus = sst.Component("memcontentbus", "memHierarchy.Bus")
memcontentbus.addParams({
   "bus_frequency" : cacheFrequency,
})
BusMemContentLink = sst.Link("bus_memcontent")
BusMemContentLink.connect((memcontentbus, "low_network_0", busLat), (comp_controller0, "contentLink", busLat))
