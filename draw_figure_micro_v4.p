...
m
els:
dby_plot=db_success_rate.plot(kind='bar')
...
def speedup_test(doMakeReport=True,doLoadData=False):
    #IPC + Speedup
    db_speedup=""
    if doLoadData == False:
        db_cycles   =   mergeCol("cycles","prospero0",id_tech)
 ...
 #       db_speedup=pd.read_csv(report_dir+"/%s-%s.csv"%("metacache_limit",result_name),index_col=0)
