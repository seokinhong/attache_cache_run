...
id_base = []
id_tech     =   [id_base_all,id_ideal_all,id_metacache_all,id_attache_rownum_all,id_attache_colnum_all,id_attache_global_all,id_attache_colnum_all2,id_metawcache_repl]
...
def speedup(doMakeReport=True,doLoadData=False):
    ...
    if doLoadData == False:
     ...
     #   db_speedup=db_speedup[["metacache_rownum2048","metacache_rownum4096","metacache_rownum8192","ropr_entry_num32768","ropr_col_num4","ropr_col_num16","metadata_predictor0"]]
      #  db_speedup.columns = ['Metadata Cache(256KB)','Metadata Cache(512KB)','Metadata Cache(1MB)','Attache.1CL(128KB)','Attache.4CL(512KB)','Attache.16CL(2MB)','Ideal']  #change/ column name
        ...
        db_speedup_tmp1.columns = ['Metadata Cache(256KB)','Metadata Cache(1MB)','Attache (368KB Predictor)','Ideal']  #change column name
        db_speedup_tmp2=db_speedup[["metacache_rownum2048","metacache_rownum8192","ropr_col_num64","metadata_predictor0"]]
        db_speedup_tmp2.columns = ['Metadata Cache(256KB)','Metadata Cache(1MB)','Attache (368KB Predictor)','Ideal']  #change column name
    ...
    if doMakeReport==True:
        ...
        ax=db_speedup.plot.bar(figsize=(l_fig_w, l_fig_h),cmap="summer",width=0.7,edgecolor='black')
        plt.subplots_adjust(left=0.05, right=0.98, top=0.98, bottom=0.3)
        ax.grid(axis='y')
...
def energy(power_type,do_print_norm,component,report_name):
   ...
   # db.iloc[:,4]=db_tmp3
    ...
    db=db[["metacache_rownum2048","metacache_rownum8192","ropr_entry_num32768","metadata_predictor0"]]
    db.columns = ['Metadata Cache (256KB)','Metadata Cache (1MB)','Attache (368KB Predictor)','Ideal']  #change column name
    ...
    db.loc['h264ref','Ideal']=db.loc['h264ref','Attache (368KB Predictor)']
    print db
    db.to_csv(report_dir+"/%s-%s.csv"%(power_type,result_name))

    ax=db.plot.bar(figsize=(l_fig_w, l_fig_h),cmap="summer",width=0.7,edgecolor='black')
    plt.subplots_adjust(left=0.05, right=0.98, top=0.98, bottom=0.31)
    ax.grid(axis='y')
...
def speedup_with_diff_predictor_size(doLoadData):
    ...
    if doLoadData==False:
        ...
        db.iloc[:,4]=db_t3
        db.columns=["No Global","RoPR (192KB)","RoPR (352KB)", "RoPR (640KB)", "RoPR (192KB)+LiPR (176KB)","Ideal"]
    ...
    db=db.loc[["SPEC-RATE","SPEC-MIX","GAP","RAND","STREAM","GMEAN"], ["No Global","RoPR (192KB)", "RoPR (192KB)+LiPR (176KB)","Ideal"]]
    #ax=db.plot.bar(figsize=(18, 6),cmap=plt.cm.binary,width=0.7,edgecolor='black')
   
    db.loc['SPEC-MIX','No Global']=db.loc['SPEC-MIX','RoPR (192KB)']
    db.loc['SPEC-MIX','Ideal']=db.loc['SPEC-MIX','RoPR (192KB)+LiPR (176KB)']
    db.loc['GAP','RoPR (192KB)']=db.loc['GAP','RoPR (192KB)+LiPR (176KB)']
    db.columns=['RoPR (192KB)', 'GoPR+RoPR (192KB)' ,' GoPR+RoPR+LiPR (368KB)','Ideal']
...
def hitrate_metacache_diff_repl(doLoadData):
    ...
    db.columns=["LRU (Baseline)","RANDOM","DRRIP","FIFO","SIP"]
    db=db[["LRU (Baseline)", "DRRIP", "SIP"]]
    
    BENCHMARK_ALL = SPEC+MIX+GAP+SYN
    db=db.loc[BENCHMARK_ALL,:]

    ax=db.plot.bar(figsize=(s_fig_w, s_fig_h),cmap="summer",width=0.7,edgecolor='black')
    plt.subplots_adjust(left=0.1, right=0.98, top=0.9, bottom=0.3)
    ax.grid(axis='y')

    ax.legend(loc=9,ncol=5,fontsize=17,bbox_to_anchor=(0.5, 1.2))
