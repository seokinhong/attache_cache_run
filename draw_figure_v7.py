...
id_base = []
id_tech     =   [id_base_all,id_ideal_all,id_metacache_all,id_attache_rownum_all,id_attache_colnum_all,id_attache_global_all,id_attache_colnum_all2,id_metawcache_repl,id_attache_rownum_all2]
...
def addtraffic_compare_intro(doLoadData):
  ...
  #  print db2
    ...
    ax = fig.add_subplot(1,1,1,axisbg="1.0")
    ax.scatter(x=db_comp_spec, y=db_spec.iloc[:,0]*100,color="b",marker="o",label="SPEC",s=100)
    ax.scatter(x=db_comp_gap,y=db_gap.iloc[:,0]*100,color="r",marker="*",label="GAP",s=100)
   ...
   # ax.scatter(x=db_comp_gap,y=db_gap.iloc[:,1],color="r",marker="o",label="GAP (Ideal)")
    ...
    plt.xticks(fontsize=18,rotation=45)
    plt.yticks(fontsize=18)
    plt.ylabel("Additional Memory Traffic (%)",fontsize=18)
    plt.xlabel("Percentage of cachelines compressible to 30B",fontsize=20)
...
#addtraffic_compare_intro(True)
#energy("totalEnergy",1,"Dimm0","Energy Consumption")  #2
#compratio(False)  #3
predic_accuracy(True) #4
