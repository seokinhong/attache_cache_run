...
comp_memhBridge.addParams({
                    "numTxnPerCycle" : coreCount,
                    "strControllerClockFreq" : coreFrequency
                    })
...
l3.addParams({
    ...
    "associativity"         : "16",
    "access_latency_cycles" : "20",    # (20 cycle * (1/3GHz) - link latency (2ns))/(1/3GHz)
...
memory.addParams({
 ...
 #   "backend" : "memHierarchy.simpleMem",
    "backend.max_outstanding_requests" : 512,
...
link_dir_cramsim_link.connect( (memory, "cube_link", busLat), (comp_memhBridge,"cpuLink", busLat) )

# Bridge <-> L3Cache
l3Link = sst.Link("L3CacheLink")
l3Link.connect((comp_memhBridge,"memLink",busLat),(comp_L3Cache,"cpuLink",busLat))
...
MemCtrlLink.connect((membus, "low_network_0", busLat), (memory, "direct_link", busLat))
...
for core in range (0, coreCount):
    ...
    L1MembusLink = sst.Link("l1_membus_%d"%core)
    L1MembusLink.connect((l1, "low_network_0", busLat), (membus, "high_network_%d"%core, busLat))
    '''
    prospero_cache_link = sst.Link("prospero_cache_link_" + str(core))
    prospero_cache_link.connect( ( prospero["prospero" + str(core)], "cache_link", busLat), (membus, "high_network_%d"%core, busLat) )
    ...
    PageAllocLink.connect((prospero["prospero" + str(core)], "pageLink",busLat), (memory, "pageLink_%d"%core, busLat));
