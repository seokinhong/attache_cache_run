...
parser.add_option("--metacache_rownum",  action='store', dest="metacache_rownum", help="The number of rows of metacache",default=1024)
parser.add_option("--predictor_entry_num",  action='store', dest="predictor_entry_num", help="The number of predictor entry",default=32*1024)
...
print "predictor_entry_num: %s"%options.predictor_entry_num
...
if options.BenchmarkType=="spec":
    ...
    mt_mode=0;
    for i in range(0,coreCount):
        trace="./%s/%s-%s0%s" % (gap_trace_dir,options.Benchmark,options.TraceType,extension);
    ...
    for i in range(0,coreCount):
        trace="./%s/%s-%s0%s" % (spec_trace_dir,benchmark_set[i],options.TraceType,extension);
       # print trace;
        traceName.append(trace)
...
comp_controller0.addParams({
                ...
                "predictor_entry_num" : options.predictor_entry_num,
